...
log "STILL MANUAL, per teammate (System Settings > Privacy & Security):"
log "  1. RECEIVING needs: grant '${OUT}' Full Disk Access."
log "  2. SENDING also needs Accessibility + Automation (the CLI drives"
log "     Messages.app via the automation/accessibility APIs — Full Disk Access"
log "     alone lets it READ but not SEND). Grant '${OUT}' Accessibility, and"
log "     allow controlling 'Messages' under Automation. Because the daemon runs"
log "     the CLI under launchd (python3), you may also need to enable python3"
log "     for Automation there."
log "  3. set self_handle (your iMessage phone/email) in imessage/daemon.json"
log "  then re-run setup.sh to load the daemon."
log "  NOTE: the CLI is ad-hoc signed — do NOT rebuild it once granted, or the"
log "  new signature hash revokes these grants. reset.sh preserves it by default."
