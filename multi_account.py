...
def instance_overlay(data, release, overlay):
    ...
    policy = "default-src 'self'; connect-src 'self' " + base + "; img-src 'self' blob:; style-src 'self' https://fonts.googleapis.com; font-src 'self' https://fonts.gstatic.com; script-src 'self'; object-src 'none'; base-uri 'none'; form-action 'none'; frame-ancestors 'none'; require-trusted-types-for 'script'; trusted-types 'none'"
    ...
    config = {'installId': data['install_id'], 'csBase': base, 'profile': 'imessage'}
    ...
    services = {k: v for k, v in overlay['services'].items() if k in ('views', 'postgres', 'synapse')}
...
def install(root):
    ...
    allowed = {'synapse' if s == 'synapse' else 'app' for s in running if s in ('synapse', 'views')}
